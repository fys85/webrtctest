...
public class RtpSender {
  /** Java wrapper for a C++ RtpSenderObserverInterface*/
  public static interface Observer {
    // Called when the first audio or video packet is send.
    @CalledByNative("Observer")
    public void onFirstPacketSend(MediaStreamTrack.MediaType media_type);
  }
  ...
  public void SetObserver(RtpSender.Observer observer) {
    checkRtpSenderExists();
    // Unset the existing one before setting a new one.
    if (nativeObserver != 0) {
      nativeUnsetObserver(nativeRtpSender, nativeObserver);
    }
    nativeObserver = nativeSetObserver(nativeRtpSender, observer);
  }
  ...
  private static native long nativeSetObserver(long rtpSender, Observer observer);

  private static native void nativeUnsetObserver(long rtpSender, long nativeObserver);
};
