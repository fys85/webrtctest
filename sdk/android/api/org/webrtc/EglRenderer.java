...
public class EglRenderer implements VideoSink {
  ...
  if (scalingType == RendererCommon.ScalingType.SCALE_ASPECT_FIT) {
      viewportWidth = Math.min(
              surfaceWidth, Math.round(surfaceHeight  * frameAspectRatio));
      viewportHeight = Math.min(
              surfaceHeight, Math.round(viewportWidth  / frameAspectRatio));
      viewportX = Math.abs(surfaceWidth - viewportWidth) / 2;
      viewportY = Math.abs(surfaceHeight - viewportHeight) / 2;
  } else {
    ...
    try {
      if (shouldRenderFrame) {
        ...
        GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);
        frameDrawer.drawFrame(frame, drawer, drawMatrix, viewportX /* viewportX */, viewportY/* viewportY */,
