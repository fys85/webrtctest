...
class RtpSenderObserverJni : public RtpSenderObserverInterface {
  public:
    RtpSenderObserverJni(JNIEnv* env, const JavaRef<jobject>& j_observer)
      : j_observer_global_(env, j_observer) {}

    ~RtpSenderObserverJni() override = default;

    void OnFirstPacketSend(cricket::MediaType media_type) override {
      JNIEnv* const env = AttachCurrentThreadIfNeeded();
      Java_Observer_onFirstPacketSend(env, j_observer_global_,
...
static jlong JNI_RtpSender_SetObserver(
            JNIEnv* jni,
            jlong j_rtp_sender_pointer,
            const JavaParamRef<jobject>& j_observer) {
        RtpSenderObserverJni* rtpSenderObserver =
                new RtpSenderObserverJni(jni, j_observer);
        reinterpret_cast<RtpSenderInterface*>(j_rtp_sender_pointer)
                ->SetObserver(rtpSenderObserver);
        return jlongFromPointer(rtpSenderObserver);
}

static void JNI_RtpSender_UnsetObserver(JNIEnv* jni,
                                              jlong j_rtp_sender_pointer,
                                              jlong j_observer_pointer) {
        reinterpret_cast<RtpSenderInterface*>(j_rtp_sender_pointer)
                ->SetObserver(nullptr);
        RtpSenderObserverJni* observer =
                reinterpret_cast<RtpSenderObserverJni*>(j_observer_pointer);
        if (observer) {
            delete observer;
        }
}
