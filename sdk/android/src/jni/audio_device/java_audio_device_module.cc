...
static jlong JNI_JavaAudioDeviceModule_CreateAudioDeviceModule(
    ...
    const JavaParamRef<jobject>& j_custom_audio_record,
  ...
  if (j_custom_audio_record.is_null())
    audio_input = std::make_unique<AudioRecordJni>(
      env, input_parameters, kHighLatencyModeDelayEstimateInMilliseconds,
      j_webrtc_audio_record);
  else
      audio_input = std::make_unique<CustomAudioRecordJni>(
              env, input_parameters, kHighLatencyModeDelayEstimateInMilliseconds,
              j_custom_audio_record);
