...
bool VoiceProcessingAudioUnit::Start() {
  ...
  if (result != noErr) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"AudioUnitFailed"
      object:[NSString stringWithFormat:@"Failed to start audio unit. Error=%ld", (long)result]];
    return false;
  } else {
