...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  if (pending_encoder_creation_) {
    ...
    if (is_encoded_frame_buffer_)
        encoder_config_.video_format.parameters.emplace("is_encoded", "true");
  ...
  force_disable_frame_dropper_ =
      ...
      (num_layers > 1 && codec.mode == VideoCodecMode::kScreensharing) || is_encoded_frame_buffer_;
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (!last_frame_info_ || video_frame.width() != last_frame_info_->width ||
    ...
    is_encoded_frame_buffer_ = video_frame.video_frame_buffer()->IsEncodedFrameBuffer();
  ...
  if (DropDueToSize(video_frame.size()) && !is_encoded_frame_buffer_) {
  ...
  if (EncoderPaused() && !is_encoded_frame_buffer_) {
    // Storing references to a native buffer risks blocking frame capture.
    if (video_frame.video_frame_buffer()->type() !=
        VideoFrameBuffer::Type::kNative) {
      if (pending_frame_)
