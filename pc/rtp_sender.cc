...
void AudioRtpSender::SetObserver(webrtc::RtpSenderObserverInterface *observer) {
    observer_ = observer;
    if (send_first_packet_ && observer_) {
        observer_->OnFirstPacketSend(media_type());
    }
}

void AudioRtpSender::NotifyFirstPacketSend() {
    if (observer_) {
        observer_->OnFirstPacketSend(media_type());
    }
    send_first_packet_ = true;
}
...
void VideoRtpSender::SetObserver(webrtc::RtpSenderObserverInterface *observer) {
    observer_ = observer;
    if (send_first_packet_ && observer_) {
        observer_->OnFirstPacketSend(media_type());
    }
...
void VideoRtpSender::NotifyFirstPacketSend() {
    if (observer_) {
        observer_->OnFirstPacketSend(media_type());
    }
    send_first_packet_ = true;
