...
void RtpTransceiver::SetChannel(cricket::ChannelInterface* channel) {
  ...
  if (channel_) {
    ...
    channel_->SignalFirstPacketSend().disconnect(this);
  }
  ...
  if (channel_) {
    ...
    channel_->SignalFirstPacketSend().connect(
        this, &RtpTransceiver::OnFirstPacketSend);
  }
...
void RtpTransceiver::OnFirstPacketSend(cricket::ChannelInterface*) {
    for (const auto& sender : senders_) {
        sender->internal()->NotifyFirstPacketSend();
    }
}
...
void RtpTransceiver::ResetFirstPacketRecvSignal() {
  for(const auto& receiver : receivers_) {
     receiver->internal()->ResetFirstPacketRecvSignal();
  }
  if (channel_)
    channel_->ResetFirstPacketRecvSignal();
}
