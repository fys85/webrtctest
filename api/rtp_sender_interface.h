...
class RtpSenderObserverInterface {
  public:
    ...
    // they will all call OnFirstPacketSend at once.
    //
    // In the future, it's likely that an RtpSender will only call
    // OnFirstPacketSend when a packet is sended specifically for its
    // SSRC/mid.
    virtual void OnFirstPacketSend(cricket::MediaType media_type) = 0;
...
class RTC_EXPORT RtpSenderInterface : public rtc::RefCountInterface {
 public:
  ...
  virtual void NotifyFirstPacketSend() = 0;
 protected:
...
PROXY_METHOD0(void, NotifyFirstPacketSend)
END_PROXY_MAP()
