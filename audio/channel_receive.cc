...
#ifdef RS_FEC
  //Reed-Solomon  //add by wuzhiren20210315
  if(header.payloadType == 115) //rsfecopus
  {
    RTPHeader header_tmp = header;
    int hlen = (int)header.headerLength;
    int buf_size = 1500; //max ip udp size
    std::unique_ptr<uint8_t> decoded_buf (new uint8_t[buf_size]);
    int decoded_size = 0;
    memcpy(decoded_buf.get(),packet_copy.data(),hlen); 
  
    header_tmp.payloadType = header.payloadType;
    header_tmp.payload_type_frequency = it->second;

    if (header_tmp.payload_type_frequency < 0)
      return;

    decoded_size = rs_decoder_->Decode(packet_copy.data()+hlen, (int)packet.size() - hlen ,
          header.sequenceNumber , header.timestamp, decoded_buf.get()+hlen, buf_size-hlen, header.payloadType);
    ...
    while((decoded_size = rs_decoder_->GetRecoveryData(decoded_buf.get()+hlen, buf_size-hlen, header_tmp.sequenceNumber , header_tmp.timestamp, header_tmp.payloadType))>0)
    {
      ReceivePacket(decoded_buf.get(), hlen+decoded_size, header_tmp);
    }
