...
#if 0
  gc_frame_.UpdateFrame(0, static_cast<int16_t*>(audioSamples), nSamples,
          samplesPerSec, AudioFrame::kNormalSpeech, AudioFrame::kVadActive, nChannels);

  processMixStreamWithGc(&gc_frame_);
  memcpy(static_cast<int16_t*>(audioSamples), gc_frame_.data(), nSamplesOut);
#endif
  return 0;
}

void AudioTransportImpl::processMixStreamWithGc(AudioFrame* audioFrame) {
    if (gc_buffer_ == nullptr ||
        gc_buffer_.get()->num_frames() != audioFrame->samples_per_channel()) {
        RTC_LOG(LS_INFO) << "create or recreate audio buffer and gain controller, sr: " << audioFrame->sample_rate_hz()
                         << " ch: " << audioFrame->num_channels();
        gc_buffer_.reset(
                new AudioBuffer(audioFrame->sample_rate_hz(), audioFrame->num_channels(),
                                audioFrame->sample_rate_hz(), audioFrame->num_channels(),
                                audioFrame->sample_rate_hz(), audioFrame->num_channels()));
        gain_control_.reset(new GainControlImpl());
        gain_control_->Initialize(audioFrame->num_channels(), audioFrame->sample_rate_hz());
        GainControl *gc = static_cast<GainControl *>(gain_control_.get());
        gc->Enable(true);
        gc->set_mode(GainControl::kAdaptiveDigital);
        gc->set_stream_analog_level(0);
        gc->set_target_level_dbfs(2);
        gc->set_compression_gain_db(50);
        gc->enable_limiter(true);
        gc->set_analog_level_limits(0, 255);
    }

    gc_buffer_->CopyFrom(audioFrame);

    if (audioFrame->sample_rate_hz() > AudioProcessing::kSampleRate16kHz) {
        gc_buffer_->SplitIntoFrequencyBands();
    }

    int ret = gain_control_->AnalyzeCaptureAudio(gc_buffer_.get());
    if (ret != AudioProcessing::kNoError) {
        RTC_LOG(LS_INFO) << "AnalyzeCaptureAudio fail, is agc enable: " << gain_control_->is_enabled();
    }
    ret = gain_control_->ProcessCaptureAudio(gc_buffer_.get(), false);
    if (ret != AudioProcessing::kNoError) {
        RTC_LOG(LS_INFO) << "ProcessCaptureAudio,is agc enable: " << gain_control_->is_enabled();
    }
    if (audioFrame->sample_rate_hz() > AudioProcessing::kSampleRate16kHz) {
        gc_buffer_->MergeFrequencyBands();
    }

    gc_buffer_->CopyTo(audioFrame);
}
