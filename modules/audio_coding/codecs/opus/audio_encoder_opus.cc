...
bool AudioEncoderOpusImpl::RecreateEncoderInstance(
  ...
  int bitrate = GetBitrateBps(config);
  if(RSFECOPUS==config.payload_type)
    bitrate = 20000;      

  //const int bitrate = 20000;//GetBitrateBps(config);//bywuzhiren20200325
...
void AudioEncoderOpusImpl::SetTargetBitrate(int bits_per_second) {
  ...
  if (config_.bitrate_bps && *config_.bitrate_bps != new_bitrate) {
    ...
    int bitrate = GetBitrateBps(config_);
    if(RSFECOPUS==config_.payload_type)
      bitrate = 20000;

    //const int bitrate = 20000;//GetBitrateBps(config_);//bywuzhiren20200325
    RTC_CHECK_EQ(0, WebRtcOpus_SetBitRate(inst_, bitrate));
    RTC_LOG(LS_INFO) << "Set Opus bitrate to " << bitrate << " bps.";
    bitrate_changed_ = true;
  }
