...
void ModuleRtpRtcpImpl::IncomingRtcpPacket(const uint8_t* rtcp_packet,
                                           const size_t length) {
  fraction_lost = 0;//add by liubin                                             
  rtcp_receiver_.fraction_lost = 0; //add by liubin 

  rtcp_receiver_.IncomingPacket(rtcp_packet, length);

  fraction_lost = rtcp_receiver_.fraction_lost;//add by liubin 
}
...
uint8_t ModuleRtpRtcpImpl::FractionLost() {
